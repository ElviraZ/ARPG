	...
	class SectorAttackSelector : IAttackSelector
	{
        public GameObject[] SelectTarget(SkillData skillData, Transform skillTransform)
        {
            ...
            array = CollectionHelper.FindAll<GameObject>(array,
                 p => 
                    Array.IndexOf(skillData.attckTargetTags, p.tag) >= 0 &&
                    p.GetComponent<CharacterStatus>().HP > 0 && 
                    Vector3.Angle(skillTransform.forward,p.transform.position - skillTransform.position) <= skillData.attackAngle /2 
                    );
